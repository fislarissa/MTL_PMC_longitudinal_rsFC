...
#functional connectivity and episodic memory in older adults
...
data_subsample_A_cognition <- data_subsample_A %>%
  filter(!(ID %in% c("sub-xxx", "sub-xxx"))) #possible MCI later
data_subsample_A_cognition <- data_subsample_A_cognition %>%
  mutate(time_BL_to_FU24 = ifelse(ID == "sub-xxx", 612, time_BL_to_FU24)) #3x follow-up after 3 months as cognition baseline
data_subsample_A_cognition <- data_subsample_A_cognition %>%
  mutate(time_BL_to_FU24 = ifelse(ID == "sub-xxx", 603, time_BL_to_FU24))
data_subsample_A_cognition <- data_subsample_A_cognition %>%
  mutate(time_BL_to_FU24 = ifelse(ID == "sub-xxx", 613, time_BL_to_FU24))
#subsample B
data_subsample_B_cognition <- data_subsample_B %>%
  filter(ID != "sub-xxx") #possible MCI later
data_subsample_B_cognition <- data_subsample_B_cognition %>%
  mutate(time_BL_to_FU24 = ifelse(ID == "sub-xxx", 602, time_BL_to_FU24)) #2x follow-up after 3 months as cognition baseline
data_subsample_B_cognition <- data_subsample_B_cognition %>%
  mutate(time_BL_to_FU24 = ifelse(ID == "sub-xxx", 612, time_BL_to_FU24))
